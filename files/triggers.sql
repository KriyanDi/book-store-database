...
-- ORDER -- 
CREATE TRIGGER CHECK_CARD_EXPIRED
AFTER UPDATE OF STATUS ON ORDER
REFERENCING NEW AS N OLD  AS O
    FOR EACH ROW
    WHEN (N.STATUS = 'ACCEPTED')
        BEGIN ATOMIC
            DECLARE CURR_CARDNO INTEGER;
            DECLARE FLAG INTEGER;
            SET CURR_CARDNO = (SELECT CARDNO
                                FROM ORDER
                                WHERE ORDERNO = N.ORDERNO);
            SET FLAG = FN45419.IS_EXPIRED(CURR_CARDNO);
            IF FLAG = 1 THEN
                UPDATE ORDER SET STATUS = 'UNACCEPTED' WHERE ORDERNO =  N.ORDERNO;
            END IF;
        END;