...
CREATE FUNCTION GET_PROD_BY_COMPANYNAME(P_COPMANYNAME CHARACTER(15))
    RETURNS
        TABLE
        (
            SKU INTEGER
...
BEGIN
    ...
    SET VENDORIDF = (SELECT ID FROM VENDOR WHERE COMPANYNAME = P_COPMANYNAME);
...
BEGIN
    ...
    DECLARE QUANTITYB INTEGER;
    DECLARE QUANTITYN INTEGER;
    DECLARE QUANTITYP INTEGER;
    DECLARE QUANTITYPL INTEGER;

    SET QUANTITYB = (SELECT STORGQUANT FROM BOOK WHERE SKU = P_SKU);
    SET QUANTITYN = (SELECT STORGQUANT FROM NOTEBOOK WHERE SKU = P_SKU);
    SET QUANTITYP = (SELECT STORGQUANT FROM PEN WHERE SKU = P_SKU);
    SET QUANTITYPL = (SELECT STORGQUANT FROM PENCIL WHERE SKU = P_SKU);

    IF (QUANTITYB > 0) THEN
        RETURN 1;
    ELSEIF (QUANTITYN > 0) THEN
        RETURN 1;
    ELSEIF (QUANTITYP > 0) THEN
        RETURN 1;
    ELSEIF (QUANTITYPL > 0) THEN
        RETURN 1;
    ELSE
        RETURN 0;
    END IF;
